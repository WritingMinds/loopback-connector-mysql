...
MySQL.prototype._modifyOrCreate = function(model, data, options, fields, cb) {
	...
	for (var i = 0, n = fields.names.length; i < n; i++) {
		if (!fields.properties[i].id) {
			setValues.push(new ParameterizedSQL(fields.names[i] + "=" + columnValues[i].sql, columnValues[i].params));
		}
	}
...
MySQL.prototype.buildExpression = function(columnName, operator, operatorValue, propertyDefinition) {
	if (operator === "regexp") {
		...
		if (operatorValue.ignoreCase) g.warn("{{MySQL}} {{regex}} syntax does not respect the {{`i`}} flag");
		if (operatorValue.global) g.warn("{{MySQL}} {{regex}} syntax does not respect the {{`g`}} flag");

		if (operatorValue.multiline) g.warn("{{MySQL}} {{regex}} syntax does not respect the {{`m`}} flag");
